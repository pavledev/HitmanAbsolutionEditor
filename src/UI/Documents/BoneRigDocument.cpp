...
BoneRigDocument::BoneRigDocument(const char* name, const char* icon, const Type type, const unsigned long long runtimeResourceID, const bool hasToolBar, const ImGuiID dockID) : Document(name, icon, type, runtimeResourceID, hasToolBar, dockID)
{
    boneRig = std::make_shared<BoneRig>();

    std::shared_ptr<SceneHierarchyPanel2> sceneHierarchyPanel = std::make_shared<SceneHierarchyPanel2>("Scene Hierarchy", ICON_MDI_VIEW_LIST, boneRig);
    ...
    std::shared_ptr<ResourceInfoPanel> resourceInfoPanel = std::make_shared<ResourceInfoPanel>("Resource Info", ICON_MDI_INFORMATION, boneRig);
    std::shared_ptr<HexViewerPanel> headerLibraryHexViewerPanel = std::make_shared<HexViewerPanel>("Header Library Hex Viewer", ICON_MDI_MEMORY, false, boneRig);
    std::shared_ptr<HexViewerPanel> resourceLibraryhexViewerPanel = std::make_shared<HexViewerPanel>("Resource Library Hex Viewer", ICON_MDI_MEMORY, true, boneRig);
    ...
    boneRig->SetResourceLoadedCallback([sceneHierarchyPanel2, modelViewerPanel2, resourceInfoPanel2, this]()
    {
...
void BoneRigDocument::CreateLayout(const ImGuiID dockspaceID, const ImVec2 dockspaceSize)
{
...
std::shared_ptr<BoneRig> BoneRigDocument::GetBoneRig() const
{
    return boneRig;
}

void BoneRigDocument::RenderMenuBar()
{
    ...
    UI::ResourceExportPopup(showResourceExportPopup, boneRig);

    ImGui::EndMenuBar();
}

void BoneRigDocument::OnResourceLoaded()
{
    boneRig->Deserialize();
}
