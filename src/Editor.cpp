...
Editor::Editor() {
  ...
  // Unique identifier to match/filter top level editors with the top level
  // dockspace. This could be a user enum.
...
void Editor::UpdateDiscordPresence() {
  ...
  if (cachedActiveDocument != lastActiveDocument || numDocuments != documents.size()) {
...
void Editor::UpdateDocumentLocation(std::shared_ptr<Document> document,
  ...
  // This should ideally be a stack so we can handle closure immediately without
  // an awkward gap where LastActiveDocument is NULL.
  if (ImGui::IsWindowFocused(ImGuiFocusedFlags_ChildWindows))
    lastActiveDocument = document;

  ImGuiWindowClass *toolWindowsClass = document->GetToolWindowsClass();
  // Set WindowClass based on per-document ID, so tabs from Document A are not
  // dockable in Document B etc. We could be using any ID suiting us, e.g. &doc
  // We also set ParentViewportId to request the platform back-end to set
  // parent/child relationship at the windowing level.
  ...
  toolWindowsClass->ParentViewportId =
      ImGui::GetWindowViewport()
          ->ID; // Make child of the top-level editor window
  ...
  // Track LocationID change so we can fork/copy the layout data according to
  // where the window is going + reference count LocationID ~~ (DockId != 0 ?
  // DockId : DocumentID) // When we are in a loose floating window we use our
  // own document id instead of the dock id
  ...
  // So all editors of a same type inside a same tab-bar will share the same
  // layout. We will also use this value as a suffix to create window titles,
  // but we could perfectly have an indirection to allocate and use nicer names
  // for window names (e.g. 0001, 0002).
...
void Editor::UpdateDocumentContents(std::shared_ptr<Document> document) {
  // This is the second Begin(), as UpdateDocumentLocationAndLayout() has
  // already done one (Therefore only the p_open and flags of the first call to
  // Begin() applies)
  ImGui::Begin(
      std::format("{}##{}", document->GetName(), document->GetID()).c_str());
  ...
  if (ImGui::GetCurrentWindow()->BeginCount !=
      2) // Case when document is created (UpdateDocumentLocation wasn't called)
  {
    ImGui::End();
  ...
  // Fork settings when extracting to a new location, or Overwrite settings when
  // docking back into an existing location
  if (document->GetPreviousLocationID() != 0 &&
    ...
    // FIXME: should be able to do a "move window but keep layout" if
    // currentDockspaceReferencecCount > 1.
    // IMGUI_DEBUG_LOG("LayoutCopy DockID %08X -> DockID %08X requested by doc
    // '%s'\n", doc->PrevDockspaceID, doc->CurrDockspaceID, doc->Name);
    // IMGUI_DEBUG_LOG("--> previousDockspaceReferencecCount = %d --> %s\n",
    // previousDockspaceReferencecCount, (previousDockspaceReferencecCount == 0)
    // ? "Remove" : "Keep"); IMGUI_DEBUG_LOG("-->
    // currentDockspaceReferencecCount = %d\n",
    ...
    // (to be able to call DockBuilderSplitNode() we need a node size ahead, so
    // any preset override will probably have to be applied between Begin and
    // Dockspace calls)
    // FIXME: Use g_LastActiveDoc as source reference and do a copy?
    SetupLayout(document, dockspaceID, dockspaceSize);
  }

  // FIXME-DOCK: This is a little tricky to explain but we currently need this
  // to use the pattern of sharing a same dockspace between tabs of a same tab
  // bar
  bool visible = true;
  ...
  if (!visible) {
    // Keep alive document dockspace so windows that are docked into it but
    // which visibility are not linked to the dockspace visibility won't get
    // undocked.
