...
const uint32_t* DecodeDeclaration(BinaryDecompiler::Shader* psShader, const uint32_t* pui32Token, Declaration* psDecl)
{
...
//
        ...
        case OPCODE_DCL_UNORDERED_ACCESS_VIEW_RAW:
        {
            ...
            BinaryDecompiler::ConstantBuffer* psBuffer = NULL;
...
const uint32_t* DeocdeInstruction(const uint32_t* pui32Token, Instruction* psInst, BinaryDecompiler::Shader* psShader)
{
    uint32_t ui32TokenLength = DecodeInstructionLength(*pui32Token);
    const uint32_t bExtended = DecodeIsOpcodeExtended(*pui32Token);
    const OPCODE_TYPE eOpcode = DecodeOpcodeType(*pui32Token);
...
const uint32_t* DecodeShaderPhase(const uint32_t* pui32Tokens,
										  BinaryDecompiler::Shader* psShader,
...
const uint32_t* DecodeHullShader(const uint32_t* pui32Tokens, BinaryDecompiler::Shader* psShader)
{
	const uint32_t* pui32CurrentToken = pui32Tokens;
	const uint32_t ui32ShaderLength = psShader->ui32ShaderLength;
...
void Decode(const uint32_t* pui32Tokens, BinaryDecompiler::Shader* psShader)
{
	const uint32_t* pui32CurrentToken = pui32Tokens;
...
BinaryDecompiler::Shader* DecodeDXBC(uint32_t* data)
{
	...
	psShader = new BinaryDecompiler::Shader();
