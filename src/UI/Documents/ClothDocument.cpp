...
ClothDocument::ClothDocument(const char* name, const char* icon, const Type type, const unsigned long long runtimeResourceID, const bool hasToolBar, const ImGuiID dockID) : Document(name, icon, type, runtimeResourceID, hasToolBar, dockID)
{
    cloth = std::make_shared<Cloth>();

    std::shared_ptr<SceneHierarchyPanel2> sceneHierarchyPanel = std::make_shared<SceneHierarchyPanel2>("Scene Hierarchy", ICON_MDI_VIEW_LIST, cloth);
    ...
    std::shared_ptr<ResourceInfoPanel> resourceInfoPanel = std::make_shared<ResourceInfoPanel>("Resource Info", ICON_MDI_INFORMATION, cloth);
    std::shared_ptr<HexViewerPanel> headerLibraryHexViewerPanel = std::make_shared<HexViewerPanel>("Header Library Hex Viewer", ICON_MDI_MEMORY, false, cloth);
    std::shared_ptr<HexViewerPanel> resourceLibraryhexViewerPanel = std::make_shared<HexViewerPanel>("Resource Library Hex Viewer", ICON_MDI_MEMORY, true, cloth);
    ...
    cloth->SetResourceLoadedCallback([sceneHierarchyPanel2, modelViewerPanel2, resourceInfoPanel2, this]()
    {
...
void ClothDocument::CreateLayout(const ImGuiID dockspaceID, const ImVec2 dockspaceSize)
{
...
std::shared_ptr<Cloth> ClothDocument::GetCloth() const
{
    return cloth;
}

void ClothDocument::RenderMenuBar()
{
    ...
    UI::ResourceExportPopup(showResourceExportPopup, cloth);

    ImGui::EndMenuBar();
}

void ClothDocument::OnResourceLoaded()
{
    cloth->Deserialize();
}
