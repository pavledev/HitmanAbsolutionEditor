...
#undef DT
	...
	string GetInterpolation(BinaryDecompiler::Shader *shader, string vRegister)
	{
	...
	void ParseInputSignature(BinaryDecompiler::Shader *shader, const char *c, size_t size)
	{
	...
	void ParseBufferDefinitions(BinaryDecompiler::Shader *shader, const char *c, size_t size)
	{
	...
	void ParseStructureDefinitions(BinaryDecompiler::Shader *shader, const char *c, size_t size)
	{
	...
	bool translate_structured_var(BinaryDecompiler::Shader *shader, const char *c, size_t &pos, size_t &size, Instruction *instr,
		...
		*combined = false;
		...
		if (group != (ResourceGroup)-1 && GetResourceFromBindingPoint(group, texture->ui32RegisterNumber, shader->sInfo, &bindInfo))
		{
			...
			if (mStructuredBufferUsedNames.find(struct_type_i->second) != mStructuredBufferUsedNames.end())
			{
				...
				{
					// Static offset:
					BinaryDecompiler::ConstantBuffer *bufInfo = NULL;
	...
	void parse_ld_structured(BinaryDecompiler::Shader *shader, const char *c, size_t &pos, size_t &size, Instruction *instr)
	{
	...
	void parse_store_structured(BinaryDecompiler::Shader *shader, const char *c, size_t &pos, size_t &size, Instruction *instr)
	{
		std::string translated[4];
		char buffer[512];
		bool combined;
	...
	void ParseCode(BinaryDecompiler::Shader *shader, const char *c, size_t size)
	{
	...
	void ParseCodeOnlyShaderType(BinaryDecompiler::Shader *shader, const char *c, size_t size)
	{
		mOutputRegisterValues.clear();
		mBooleanRegisters.clear();
		mCodeStartPos = mOutput.size();
...
const string DecompileBinaryHLSL(ParseParameters &params, bool &patched, std::string &shaderModel, bool &errorOccurred)
{
	...
	try
	{
		BinaryDecompiler::Shader *shader = DecodeDXBC((uint32_t*)params.bytecode);
	...
	catch (...)
	{
		...
		LogInfo("   ******* Exception caught while decompiling BinaryDecompiler::Shader ******\n");
