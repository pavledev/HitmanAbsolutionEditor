...
PhysicsDocument::PhysicsDocument(const char* name, const char* icon, const Type type, const unsigned long long runtimeResourceID, const bool hasToolBar, const ImGuiID dockID) : Document(name, icon, type, runtimeResourceID, hasToolBar, dockID)
{
    physics = std::make_shared<Physics>();

    std::shared_ptr<SceneHierarchyPanel2> sceneHierarchyPanel = std::make_shared<SceneHierarchyPanel2>("Scene Hierarchy", ICON_MDI_VIEW_LIST, physics);
    ...
    std::shared_ptr<ResourceInfoPanel> resourceInfoPanel = std::make_shared<ResourceInfoPanel>("Resource Info", ICON_MDI_INFORMATION, physics);
    std::shared_ptr<HexViewerPanel> headerLibraryHexViewerPanel = std::make_shared<HexViewerPanel>("Header Library Hex Viewer", ICON_MDI_MEMORY, false, physics);
    std::shared_ptr<HexViewerPanel> resourceLibraryhexViewerPanel = std::make_shared<HexViewerPanel>("Resource Library Hex Viewer", ICON_MDI_MEMORY, true, physics);
    ...
    physics->SetResourceLoadedCallback([sceneHierarchyPanel2, modelViewerPanel2, resourceInfoPanel2, this]()
    {
...
void PhysicsDocument::CreateLayout(const ImGuiID dockspaceID, const ImVec2 dockspaceSize)
{
...
std::shared_ptr<Physics> PhysicsDocument::GetPhysics() const
{
    return physics;
}

void PhysicsDocument::RenderMenuBar()
{
    ...
    UI::ResourceExportPopup(showResourceExportPopup, physics);

    ImGui::EndMenuBar();
}

void PhysicsDocument::OnResourceLoaded()
{
    physics->Deserialize();
}
