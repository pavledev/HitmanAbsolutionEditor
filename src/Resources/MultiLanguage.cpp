...
void MultiLanguage::ImportFromJson(const std::string& jsonFilePath)
{
	...
	Logger::GetInstance().Log(Logger::Level::Info, std::format("Imported multilanguage from {} ({} entries)", jsonFilePath, locales.size()));
}

bool MultiLanguage::SerializeToBinary(std::vector<unsigned char>& outputBuffer)
{
...
bool MultiLanguage::PatchResourceLibrary()
{
	...
	{
		Logger::GetInstance().Log(Logger::Level::Error, "Failed to serialize multilanguage to binary");
