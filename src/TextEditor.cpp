...
TextEditor::Coordinates TextEditor::SanitizeCoordinates(const Coordinates & aValue) const
{
	...
	{
		column = mLines.empty() ? 0 : Math::Min(column, GetLineMaxColumn(line));
		return Coordinates(line, column);
	}
}
...
void TextEditor::Advance(Coordinates & aCoordinates) const
{
	if (aCoordinates.mLine < (int)mLines.size())
	{
		...
		if (cindex + 1 < (int)line.size())
		{
			...
			cindex = Math::Min(cindex + delta, (int)line.size() - 1);
		}
		else
		{
			++aCoordinates.mLine;
...
TextEditor::Coordinates TextEditor::ScreenPosToCoordinates(const ImVec2& aPosition) const
{
	...
	int lineNo = Math::Max(0, (int)floor(local.y / mCharAdvance.y));
...
TextEditor::Coordinates TextEditor::FindNextWord(const Coordinates & aFrom) const
{
	...
	while (!isword || skip)
	{
		...
		{
			auto l = Math::Max(0, (int) mLines.size() - 1);
...
void TextEditor::Render()
{
	...
	auto lineMax = Math::Max(0, Math::Min((int)mLines.size() - 1, lineNo + (int)floor((scrollY + contentSize.y) / mCharAdvance.y)));
	...
	if (!mLines.empty())
	{
		...
		while (lineNo <= lineMax)
		{
			...
			longest = Math::Max(mTextStart + TextDistanceToLineStart(Coordinates(lineNo, GetLineMaxColumn(lineNo))), longest);
			auto columnNo = 0;
...
void TextEditor::InsertText(const char * aValue)
{
	...
	auto start = Math::Min(pos, mState.mSelectionStart);
...
void TextEditor::MoveDown(int aAmount, bool aSelect)
{
	...
	auto oldPos = mState.mCursorPosition;
	mState.mCursorPosition.mLine = Math::Max(0, Math::Min((int)mLines.size() - 1, mState.mCursorPosition.mLine + aAmount));
...
void TextEditor::MoveRight(int aAmount, bool aSelect, bool aWordMode)
{
	...
	while (aAmount-- > 0)
	{
		...
		if (cindex >= line.size())
		{
			...
			{
				mState.mCursorPosition.mLine = Math::Max(0, Math::Min((int)mLines.size() - 1, mState.mCursorPosition.mLine + 1));
...
const TextEditor::Palette & TextEditor::GetDarkPalette()
{
	const static Palette p = { {
			...
			0xFFA8CEB5,	// Number
			0xFF7891CE,	// String
			0xFFFEDC9C, // Property Name
			0xFFD4D4D4, // Punctuation
			0xff101010, // Background
			0xffe0e0e0, // Cursor
			0x80a06020, // Selection
			0xFFD1D1D1, // Line number
...
{
	int toLine = aLines == -1 ? (int)mLines.size() : Math::Min((int)mLines.size(), aFromLine + aLines);
...
void TextEditor::ColorizeRange(int aFromLine, int aToLine)
{
	...
	int endLine = Math::Max(0, Math::Min((int)mLines.size(), aToLine));
	for (int i = aFromLine; i < endLine; ++i)
	{
		...
		for (auto first = bufferBegin; first != last; )
		{
			...
			else
			{
				...
				if (token_color == PaletteIndex::Keyword)
				{
					...
					if (mLanguageDefinition.mKeywords.count(id) == 0)
						token_color = PaletteIndex::Default;
				}
...
void TextEditor::ColorizeInternal()
{
	...
	if (mColorRangeMin < mColorRangeMax)
	{
		...
		const int to = Math::Min(mColorRangeMin + increment, mColorRangeMax);
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::JSON()
{
	...
	if (!inited)
	{
		...
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("\"([^\"]+)\":", PaletteIndex::PropertyName));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("L?\\\"(\\\\.|[^\\\"])*\\\"", PaletteIndex::String));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("\\\'[^\\\']*\\\'", PaletteIndex::String));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("0[xX][0-9a-fA-F]+[uU]?[lL]?[lL]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?[fF]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("[+-]?[0-9]+[Uu]?[lL]?[lL]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("[a-zA-Z_][a-zA-Z0-9_]*", PaletteIndex::Keyword));
