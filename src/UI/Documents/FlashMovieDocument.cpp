...
{
    flashMovie = std::make_shared<FlashMovie>();
    sliderValue = 0.f;
    ...
    {
        textureViewerPanel = std::make_shared<TextureViewerPanel>("Texture Viewer", ICON_MDI_IMAGE, nullptr, &sliderValue);

        AddPanel(textureViewerPanel);
    }

    std::shared_ptr<ResourceInfoPanel> resourceInfoPanel = std::make_shared<ResourceInfoPanel>("Resource Info", ICON_MDI_INFORMATION, flashMovie);
    std::shared_ptr<HexViewerPanel> headerLibraryHexViewerPanel = std::make_shared<HexViewerPanel>("Header Library Hex Viewer", ICON_MDI_MEMORY, false, flashMovie);
    std::shared_ptr<HexViewerPanel> resourceLibraryhexViewerPanel = std::make_shared<HexViewerPanel>("Resource Library Hex Viewer", ICON_MDI_MEMORY, true, flashMovie);
    ...
    flashMovie->SetResourceLoadedCallback([textureViewerPanel2, resourceInfoPanel2, this]()
    {
...
void FlashMovieDocument::CreateLayout(const ImGuiID dockspaceID, const ImVec2 dockspaceSize)
{
...
std::shared_ptr<FlashMovie> FlashMovieDocument::GetFlashMovie() const
{
    return flashMovie;
}
...
void FlashMovieDocument::RenderToolBar()
{
...
void FlashMovieDocument::OnResourceLoaded()
{
    ...
    if (!hasSwfFile)
    {
        ...
        flashMovie->CreateTextureFromMemory();

        textureViewerPanel->SetTextureWidth(flashMovie->GetTextureWidth());
        textureViewerPanel->SetTextureHeight(flashMovie->GetTextureHeight());
        textureViewerPanel->SetTexture(flashMovie->GetTexture());
        textureViewerPanel->SetTextureView(flashMovie->GetTextureView());
    }
}

void FlashMovieDocument::RenderZoomPercentageText() const
{
    std::shared_ptr<TextureViewerPanel> textureViewerPanel = std::static_pointer_cast<TextureViewerPanel>(panels[0]);
...
void FlashMovieDocument::RenderRGBAChannelButtons() const
{
    std::shared_ptr<TextureViewerPanel> textureViewerPanel = std::static_pointer_cast<TextureViewerPanel>(panels[0]);
